...
export function WhitePaper(){
  ...
  const state=useQuery(assetStateQueryOptions)
  ...
  return <main className="white-paper">
    ...
    <WpSection title="2. Charakterystyka rUSD"><p>rUSD jest demonstracyjnym tokenem inspirowanym EMT, odnoszącym się do jednej waluty urzędowej — dolara amerykańskiego. System stosuje mocne uproszczenie 1 rUSD = 1 USD oraz kurs USD/EUR 1:1 wyłącznie dla demonstracji progów raportowych.</p><Facts items={[["Nazwa",snapshot?.name??"—"],["Symbol",snapshot?.symbol??"rUSD"],["Podaż",snapshot?`${formatTokenAmount(snapshot.totalSupplyRaw,snapshot.decimals)} rUSD`:"—"],["Stan",state.data?assetStateLabel(state.data.state):"—"]]}/></WpSection>
